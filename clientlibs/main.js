...
app.registerExtension({
    ...
    async nodeCreated(node) {
        ...
        } else if(nodeClass === RUNWARE_NODE_TYPES.IMAGEINFERENCEADVANCEDFEATURES) {
            imageInferenceAdvancedFeaturesToggleHandler(node);
        } else if(nodeClass === RUNWARE_NODE_TYPES.WATERMARKADVANCEDFEATURE) {
            watermarkAdvancedFeatureToggleHandler(node);
        } else if(nodeClass === RUNWARE_NODE_TYPES.VIDEOINFERENCESPEECHINPUT) {
            videoInferenceSpeechInputToggleHandler(node);
        } else if(nodeClass === RUNWARE_NODE_TYPES.REGIONALPROMPTINGADVFEATUREREGIONS) {
            regionalPromptingRegionsToggleHandler(node);
